...
geom_line(size = 0.5) + ggtitle("Liability Increasing Mutation") + labs(x ="generation", y="average π") + 
...
print(ggplot (df, aes(x = gens, y = as.numeric(mean_pi_m2)))) +
    geom_line(size = 0.5) + ggtitle("Liability Decreasing Mutation") + labs(x ="generation", y="average π") + 
    xlim(100,20000) + geom_vline(xintercept = 10000, color="red")
...
print(ggplot (df, aes(x = gens, y = as.numeric(mean_variance)))) +
  geom_line(size = 0.5) + ggtitle("Additive Genetic Variance") + labs(x ="generation", y="additive genetic variance") +
  xlim(firstGen,lastGen) + geom_vline(xintercept = 10000, color="red")



df <- data.frame(x = gens, y = as.numeric(unlist(mean_heritability)))
print(ggplot (df, aes(x = gens, y = as.numeric(mean_heritability)))) +
  geom_line(size = 0.5) + ggtitle("Heritability") + labs(x ="generation", y="heritability") + 
  xlim(firstGen,lastGen) + geom_vline(xintercept = 10000, color="red")
...
geom_line(size = 0.5) + ggtitle("Disease Prevalence") + labs(x ="generation", y=" % prevalence") + 
xlim(firstGen,lastGen) + geom_vline(xintercept = 10000, color="red")
